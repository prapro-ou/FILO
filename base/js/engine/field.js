class Field {
    ...
    setPlayer(player) {
        this.player = player;
        this.player.x = this._map2canvas(player.mapX, this.x);
        this.player.y = this._map2canvas(player.mapY, this.y);
        this.player.defaultX = this.player.x;
        this.player.defaultY = this.player.y;
        this.player.addEventListener('changeSpeed', (e) => {
            this._setSpeed(e.target);
        });
        this.player.addEventListener('getAttr', (e) => {
            this.player.mapAttrs = this.getAttribute(e.target.mapX, e.target.mapY, e.target.fit);
        });

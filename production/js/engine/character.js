...
class NonPlayer extends Character {
    constructor(sheet, width, height, mapX, mapY, speed, animation) {
        super(sheet, width, height, mapX, mapY, speed, animation);
        this._idealMapX = mapX;
        this._idealMapY = mapY;
        this.route = [];
    }

    despawn() {
        this.dispatchEvent('despawn');
    }

    walk(mapX, mapY) {
        let direction;

        direction = (this._idealMapX < mapX) ? 1 : 3;
        while (this._idealMapX !== mapX) {
            this.route.push(direction);
            this._idealMapX += (this._idealMapX < mapX) ? 0.5 : -0.5;
        }

        direction = (this._idealMapY < mapY) ? 2 : 0;
        while (this._idealMapY !== mapY) {
            this.route.push(direction);
            this._idealMapY += (this._idealMapY < mapY) ? 0.5 : -0.5;
        }
    }
    ...
    changeDirection() {
        if (this.mapX % this._moveAmount === 0 && this.mapY % this._moveAmount === 0) {
            const direction = Math.abs(this.route[0]);
